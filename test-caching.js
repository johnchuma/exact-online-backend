  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      try {
        ...
        console.log(
          `   ⏱️  First request (DB): ${time1}ms - Status: ${response1.status}`
        );
      } catch (error) {
        console.log(
      ...
      try {
        ...
        console.log(
          `   ⚡ Second request (Cache): ${time2}ms - Status: ${response2.status}`
        );

        // Calculate performance improvement
        const improvement =
          time1 > 0 ? (((time1 - time2) / time1) * 100).toFixed(1) : 0;
    ...
    console.log(
      `   📊 Keys after product invalidation: ${afterProductInvalidation.length}`
    );

    await redisClient.invalidatePattern("*");
    const afterFullInvalidation = await redisClient.client.keys("*");
    console.log(
      `   📊 Keys after full invalidation: ${afterFullInvalidation.length}`
    );
